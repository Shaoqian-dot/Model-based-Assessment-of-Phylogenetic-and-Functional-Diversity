PD_FDSimulation <- function (alpha, beta, q, m, m_less, NOPS, r, diversity,
  ...
  for (k in 1 : length(Methods)) {
    ...
    Model_p_mid_J <- fit_model(type = 'p_mid', matrix_type = 'P_J', rr = FALSE,
                                    yX = yX, P = P, P_J = P_J, val_num.eig = val_num.eig, 
                                    Distribution = Distribution, null = FALSE)
    
    Model_p_mid_J_rr <- fit_model(type = 'p_mid', matrix_type = 'P_J', rr = TRUE,
                                   yX = yX, P = P, P_J = P_J, val_num.eig = val_num.eig, 
                                   Distribution = Distribution, null = FALSE)
    
    # Model_p_mid_J <-  Model_p_mid_J_alt
    # 
    # Model_p_mid_J_rr <- Model_p_mid_J_rr_alt
    
    #anova(Model_p_mid_J_null, Model_p_mid_J_alt)
    #anova(Model_p_mid_J_rr_null, Model_p_mid_J_rr_alt)
    ...
    models_list <- list(
      Model_p_mid_J, Model_p_mid_J_rr
    )
    ...
    # ===== Compute p values =====
    p_Model_Matrix <- matrix(NA, q - 1, n_Model)
    
    dimnames(p_Model_Matrix) <- list(
      row    = paste0("com", 2:q),
      col    = Model_label
    )
    ...
    for(j in 2 : q){
      ...
      # label_Model_3456910 <- paste0('com', j, ':p', 1 : val_num.eig) 
      ...
      # chi_square_3  <- getChisquare_fixed (Model = Model_p_J,              label = label_Model_3456910)
      # chi_square_4  <- getChisquare_fixed (Model = Model_p_J_rr,           label = label_Model_3456910)
      # chi_square_5  <- getChisquare_fixed (Model = Model_p_fix_J,          label = label_Model_3456910)
      # chi_square_6  <- getChisquare_fixed (Model = Model_p_fix_J_rr,       label = label_Model_3456910)
      ...
      # chi_square_9  <- getChisquare_fixed (Model = Model_p_mid_J,          label = label_Model_3456910)
      # chi_square_10 <- getChisquare_fixed (Model = Model_p_mid_J_rr,       label = label_Model_3456910)
    ...
    # rm(Model_p_mid_J, Model_p_mid_J)
    # gc()
  }
